...
import type { RomiMessage, RomiApiResponse, SiteContext } from './types';
...
function parseBotText(raw: string): { text: string; redirectUrl?: string; suggestions: string[] } {
...
export function useRomi({ apiUrl, siteContext, welcomeMessage }: UseRomiOptions) {
  const [messages, setMessages] = useState<RomiMessage[]>(() => {
