...
import { MessageSquare, Settings, Download, Share2, User, Bot, ArrowUp, Trash2, ChevronLeft, ChevronRight, Plus, Sun, Moon, Search, Cpu, Wrench, Lightbulb, CornerDownRight, X, BookOpen, Edit2, MapPin, ArrowUpRight, Sparkles, Hand, Heart, ArrowRight, Database, Activity } from 'lucide-react';

import RomiThinkingIndicator from './RomiPortalFeatures/RomiThinkingIndicator';
import InstrumentMapPanel, { InstLocation } from './RomiPortalFeatures/InstrumentMapPanel';
    ...
    if (resData.report_ready) {
      const download = async (format: 'docx' | 'pdf') => {
        const res = await fetch(`${apiUrl}/api/generate-report`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ session_id: targetSid, format }),
        });
...
<div
key={session.id}
className={`group flex items-center justify-between rounded-2xl px-3 py-2.5 transition-all cursor-pointer ${
isActive
...
{/* Floating Progress Bar on Top */}
{mode === 'researchpreneurship' && (
<div className="w-full px-4 md:px-8 pt-4 pb-2 bg-gradient-to-b from-[#FDFDF9] via-[#FDFDF9]/95 to-transparent dark:from-zinc-950 dark:via-zinc-950/95 dark:to-transparent z-30 shrink-0 border-b border-gray-100/50 dark:border-zinc-900/50 shadow-xs">
