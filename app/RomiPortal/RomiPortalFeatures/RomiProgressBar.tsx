...
export default function RomiProgressBar({
  ...
  return (
    <div className="bg-white border border-gray-100 shadow-sm rounded-2xl p-5 w-full">
      ...
      {/* Main Bar */}
      <div className="relative h-4 bg-gray-100 rounded-full overflow-hidden mb-6 mt-2 border border-gray-200">
        <motion.div 
        ...
        />
        <div className="absolute right-3 top-1/2 -translate-y-1/2 text-[9px] font-bold text-[#1b60bb] font-helios">
        ...
        {stages.map((stage, i) => (
          <div 
            ...
            className={`flex-1 p-3 rounded-xl border transition-all duration-300 ${
              stage.status === 'complete' ? 'bg-green-50/40 border-green-100 text-green-900' :
              stage.status === 'active' ? 'bg-blue-50/50 border-blue-200 text-blue-900 shadow-sm animate-pulse' :
          ...
          >
            <div className="flex items-center gap-2 mb-1">
              ...
              {stage.status === 'active' && <AlertCircle size={14} className="text-[#1b60bb] shrink-0" />}
              {stage.status === 'pending' && <Circle size={14} className="text-gray-300 shrink-0" />}
